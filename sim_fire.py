...
# proto_size = 768
# num_classes = 100
# proto_dict = np.load("/home/dell/XieJiahao/back_PTM_CL/poison_ptm/proto_dict/proto_dict_rever_1.npy", allow_pickle=True).item()
#
# # 目标类别和相似度约束
# target_class = 1
# target_proto = proto_dict[3]
# target_cos_sim = 0.8
#
# # 初始化proto_optim
...
def compute_loss(proto_optim, proto_dict, target_proto, optim_label, target_label):
    initial_temp = 10.0
    cooling_rate = 0.99
    num_restarts = 1
    max_iters_per_restart = 20000
    ...
    for i, proto in proto_dict.items():
        if i != optim_label and i != target_class:
...
def simulated_annealing(proto_optim, proto_dict, target_proto, optim_label, target_label):
    initial_temp = 10.0
    cooling_rate = 0.99
    num_restarts = 1
    max_iters_per_restart = 20000
    ...
    best_loss = compute_loss(proto_optim, proto_dict, target_proto, optim_label, target_label)
    temp = initial_temp
    ...
    for i in range(max_iters_per_restart):
        ...
        # 计算损失
        new_loss = compute_loss(new_proto, proto_dict, target_proto, optim_label, target_label )
